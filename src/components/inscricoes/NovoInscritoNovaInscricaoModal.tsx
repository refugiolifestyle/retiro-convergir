...
export const NovoInscritoNovaInscricaoModal: FC<Props> = ({ numeroRestanteDeParcelas, salvarInscrito }) => {
    ...
    const adicionarInscrito = useCallback(() => {
        ...
        if (!cpf || !celula || !parcelas || !nomeRef) {
            toast?.current?.show({
        ...
        if (primeiraParcela) {
            ...
            if (!cargo || !nome || !sexo || !dataNascimento || !telefone) {
                toast?.current?.show({
                    summary: 'Erro ao avançar',
                    detail: 'Campos precisam ser preenchidos',
                    severity: 'error'
            ...
            inscrito = { ...inscrito, cargo, nome, sexo, dataNascimento, telefone, observacao };
        } else {
            const nome = nomeRef;
